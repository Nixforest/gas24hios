...
//        ConfigBean(id: DomainConst.ACTION_TYPE_SELECT_GAS, name: DomainConst.CONTENT00485),
    ...
    /** Rating separator label 2*/
    var lblRatingSeparator2:         UILabel         = UILabel()
    /** Rating bar */
    ...
    /** Flag check if button order was tapped */
    public static var  _isFirstUse:                           Bool    = true
...
//        processingView?.isHidden = true
    ...
    private func showHideFinishMode(isShow: Bool) {
        lblRatingSeparator2.isHidden = !isShow
        lblRatingSeparator.isHidden = !isShow
        lblRating.isHidden = !isShow
        ratingBar.isHidden = !isShow
        txtNote.isHidden = !isShow
        btnSubmit.isHidden = !isShow
...
//    }
     ...
     * Create group Rating
     */
    private func createRatingViewHD() {
//        self.createLabel(label: lblFinish1,
//                         offset: btnFinish.frame.maxY + GlobalConst.MARGIN,
//                         text: DomainConst.CONTENT00489.uppercased(),
//                         color: GlobalConst.MAIN_COLOR_GAS_24H,
...
//            w: ORDER_BUTTON_REAL_SIZE_FHD_L, h: ORDER_BUTTON_REAL_SIZE_FHD_L)
        createRatingLabel()
        createRatingBar(h: 30)
        createNoteTextView()
        createSubmitBtn(w: lblRating.frame.width + 50, h: GlobalConst.LABEL_H * 2)
        self.view.addSubview(lblRatingSeparator2)
        self.view.addSubview(lblRatingSeparator)
    ...
    private func createRatingLabel() {
        ...
        lblRating.frame = CGRect(x: (UIScreen.main.bounds.width - width) / 2, y: lblOrder.frame.maxY + GlobalConst.MARGIN, width: width, height: GlobalConst.LABEL_H * 2)
...
//            height: GlobalConst.LABEL_H * 2)
        ...
        lblRatingSeparator.frame          = CGRect(
            x: GlobalConst.MARGIN, y: lblOrder.frame.maxY + GlobalConst.MARGIN,
            width: (UIScreen.main.bounds.width / 2) - lblRating.frame.width / 2 - GlobalConst.MARGIN,
            height: GlobalConst.LABEL_H * 2)
        lblRatingSeparator2.text           = DomainConst.SEPARATOR_STR
        lblRatingSeparator2.textColor      = GlobalConst.MAIN_COLOR_GAS_24H
        lblRatingSeparator2.font           = GlobalConst.BASE_BOLD_FONT
        lblRatingSeparator2.textAlignment  = .center
        lblRatingSeparator2.lineBreakMode  = .byCharWrapping
        lblRatingSeparator2.frame          = CGRect(
            x: lblRating.frame.maxX , y: lblOrder.frame.maxY + GlobalConst.MARGIN,
            width: (UIScreen.main.bounds.width / 2) - lblRating.frame.width / 2 - GlobalConst.MARGIN,
            height: GlobalConst.LABEL_H * 2)
    }
    ...
    private func createRatingBar(h: CGFloat) {
        ...
        ratingBar.frame = CGRect(x: (UIScreen.main.bounds.width - ratingWidth) / 2,
                                 y: lblRating.frame.maxY + GlobalConst.MARGIN,
                                 width: ratingWidth,
                                 height: h)
        ratingBar.setRatingValue(value: 0)
        //ratingBar.setBackgroundColor(color: GlobalConst.MAIN_COLOR_GAS_24H)
    ...
    private func createNoteTextView() {
        txtNote.frame = CGRect(x: GlobalConst.MARGIN,
                               ...
                               width: UIScreen.main.bounds.width - 2 * GlobalConst.MARGIN,
                               height: GlobalConst.EDITTEXT_H * 3)
        txtNote.font = GlobalConst.BASE_FONT
        txtNote.text = DomainConst.CONTENT00548
...
//    }
    ...
    private func createSubmitBtn(w: CGFloat, h: CGFloat) {
        btnSubmit.frame = CGRect(x: (UIScreen.main.bounds.width - w) / 2,
                                 y: txtNote.frame.maxY + GlobalConst.MARGIN,
                                 width: w, height: h)
        btnSubmit.setTitle(DomainConst.CONTENT00547.uppercased(), for: UIControlState())
    ...
    internal func finishRating(_ notification: Notification) {
        ...
        if model.isSuccess() {
            changeMode(value: OrderStatusEnum.STATUS_CREATE)
            ratingBar.setRatingValue(value: 0)
            txtNote.text = DomainConst.BLANK
...
extension G12F01S01VC: RatingBarDelegate {
    func rating(_ sender: AnyObject) {
        // Do nothing
    }
}
// MARK: Protocol - UITextViewDelegate
extension G12F01S01VC: UITextViewDelegate {
    func textViewShouldBeginEditing(_ textView: UITextView) -> Bool {
        if textView.text == DomainConst.CONTENT00548 {
    ...
    func textViewShouldEndEditing(_ textView: UITextView) -> Bool {
        if textView.text == DomainConst.BLANK {
            textView.text = DomainConst.CONTENT00548
        }
        return true
    }
